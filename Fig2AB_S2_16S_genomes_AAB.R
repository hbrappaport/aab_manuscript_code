...
## Code for supp figure data
count_asv_species = aab_table_sub %>%
  group_by(genus, species, sample_id) %>%
  summarize(spp_ab = (sum(rel_abun)*100) ) %>%
  ungroup() %>%
  group_by(genus, species) 

mean_ra = count_asv_species %>%
  summarize((mean(spp_ab)*100) ) 

count_asv_species_RA = count_asv_species %>%
  left_join(mean_ra)


# plot all points for SUPP figure S2
ggplot(count_asv_species_RA, aes(fct_reorder(species,`(mean(spp_ab) * 100)`),
                                 spp_ab)) +
  geom_point(pch=21, fill="#A2A9C7", color="black") + 
  labs(x="ASV taxon assignment", y = "Mean % relative abundance") +
  coord_flip() +
  theme_classic()

## code to generate main figure 2
count_asv_species = aab_table_sub %>%
  group_by(genus, species, asv_id) %>%
  summarize(mean_ra = (mean(rel_abun)*100) ) %>%
  filter(mean_ra > 0.01) %>%
  ungroup()

# relative abundance of ASVs (across samples w/any AAB detected) (Fig. 2A)
...
#------------------------------------------------------------
# look at number of LAB / Yeast in 147
#------------------------------------------------------------

# only samples with abundant AAB
input_aab_thresh_10 = filter_samples_by_counts(input_aab, min_seqs = 0.01)
